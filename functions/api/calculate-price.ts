...
import { corsHeaders, successResponse, errorResponse, now } from './utils'
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    // 获取会员配置
    const memberConfig = await db.select().from(memberConfigs).where(eq(memberConfigs.storeId, storeId)).get()
